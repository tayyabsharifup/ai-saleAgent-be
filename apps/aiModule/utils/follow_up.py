...
class AITool:
    ...
    def initialDecideNode(self, state: SalesState) -> Literal['initial_response', 'classifier', '__end__']:
        if not self.messages:
            ...
            llm = ChatOpenAI(model='gpt-4.1-mini', temperature=0).with_structured_output(ShouldFollowUp)
            system_prompt = f"""
    ...
    def interestNode(self, state: SalesState):
        llm = ChatOpenAI(model='gpt-4.1-mini', temperature=0).with_structured_output(InterestLevel)
        system_prompt = """
    ...
    def followUpNode(self, state: SalesState):
        llm = ChatOpenAI(model='gpt-4.1', temperature=0).with_structured_output(FollowUpPlan)
        system_prompt = """
